  ...
  if (!isnan(speedOfObject)) {
    ...
    if (speedCount >= 1) {
      ...
      if (motor_flag == 1) {
        ...
        motorActivated = true; // Set flag to prevent proximity measurement
      } else if (motor_flag == 2) {
        digitalWrite(motorPin, LOW);
        motorActivated = false; // Reset flag to enable proximity measurement
      }

      // Transmit data to NodeMCU via Serial communication
      Serial.print("Average Speed (over 1 readings): ");
